...
const TabMenu = (props) => {
    ...
    const _renderTabs = () => {
        return tabs.map((tab) => (React.createElement("div", { className: activeTab === tab.key ? "tab_menu_tab-active" : "tab_menu_tab", key: tab.title, onClick: () => _handleTabChange(tab.key) }, tab.title)));
    };
    const { onSearch } = props;
    return (React.createElement(Layout_1.Row, null,
        React.createElement("div", null, _renderTabs()),
